...
from app.schemas.pdf_odi import PDFOdiRequest, OdiRow
...
class PDFOdiGenerator:
    ...
    def generate_pdf(self, data: PDFOdiRequest) -> str:
        ...
        # Tabla de elementos
        story.extend(self._create_table_by_task(data.elementos))
    ...
    #TODO hacer como una tabla para cada tarea

    def _create_table(self, elementos: List[OdiRow]) -> List:
        ...
        for elemento in elementos:
            data.append([elemento.riesgo, elemento.consecuencias, elemento.precaucion])
        
        # Crear la tabla
        table = Table(data, colWidths=[2.16*inch, 2.16*inch, 2.18*inch])
    ...
    def _create_table_by_task(self, elementos: List[OdiRow]):
        ...
        for tarea in sorted(groups.keys()):
            rows = groups[tarea]
            heading = Paragraph(f"TAREA: {tarea}", self.table_title_style)
            table = self._create_table(rows)

            block = KeepTogether([heading, Spacer(1, 6), table])
            #story.append([block])
            #story.append([Spacer(1, 20)])
            story.extend([heading, table, Spacer(1, 12)])
        return story
